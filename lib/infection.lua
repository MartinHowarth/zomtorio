-- S4/S5 — infection of buildings, robots (R-INF) and (later, S5) the player.
--
...
--- Infect-on-hit. A single hit from the enemy force infects a non-wall building.
function infection.on_entity_damaged(event)
  -- Never let our own DoT re-trigger infection.
  ...
  -- The hit must be enemy-caused: the killing force is the enemy force, or the
  -- (valid) cause entity is on it.
  local enemy_hit = util.is_enemy_force(event.force)
  if not enemy_hit then
    local cause = event.cause
    enemy_hit = cause and cause.valid and util.is_enemy_force(cause.force)
  end
  if not enemy_hit then return end
